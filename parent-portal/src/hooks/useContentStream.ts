...
function invalidateParentContentTags(event: ContentUpdatedEvent) {
  const tags: Parameters<typeof api.util.invalidateTags>[0] = ["Children", "Inbox"];
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      try {
        ...
        const es = new EventSource(`/api/parent/stream?token=${encodeURIComponent(token)}`);
        ...
        es.addEventListener("content", (message) => {
          try {
            ...
            if (event?.type === "content_updated") {
              invalidateParentContentTags(event);
            }
          } catch {
