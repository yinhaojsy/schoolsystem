          ...
          {filteredStudents.length === 0 ? (
            ...
            filteredStudents.map((student) => {
              const householdLabel =
                student.householdLabel?.trim() ||
                (student.householdId != null ? `#${student.householdId}` : "—");
              return (
                <li
                  key={student.id}
                ...
                >
                  ...
                  <dl className="mt-3 space-y-1.5 text-sm text-slate-600">
                    <div className="flex justify-between gap-3">
                      ...
                      <dd className="font-medium text-slate-800 text-right">{student.classGroupName || "—"}</dd>
                    </div>
                    <div className="flex justify-between gap-3">
                      <dt className="text-slate-500">Parents</dt>
                      <dd className="text-right truncate max-w-[60%]">{student.parentsName || "—"}</dd>
                    </div>
                    <div className="flex justify-between gap-3">
                      <dt className="text-slate-500">Contact</dt>
                      <dd className="text-right">{student.contactNo || "—"}</dd>
                    </div>
                    <div className="flex justify-between gap-3">
                      <dt className="text-slate-500">Household</dt>
                      <dd className="text-right truncate max-w-[60%]">{householdLabel}</dd>
                    </div>
                    <div className="flex justify-between gap-3">
                      <dt className="text-slate-500">Fees</dt>
                      <dd className="text-right text-slate-800">
                        {student.feeStructureName} · Rs {formatRs(student.monthlyFee ?? 0)}
                      </dd>
                    </div>
                  </dl>
                  <div className="mt-3 space-y-2">
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => handleManageFeeOverrides(student)}
                        className="rounded-lg border border-purple-200 bg-purple-50 py-2.5 text-sm font-semibold text-purple-800"
                      >
                        Manage fees
                      </button>
                    </div>
